...
module.exports = function(theme) {
  if (theme) {
    compileIndex(theme, GLOBAL.topshelfGlobals.extensions);
  } else {
    Themes.find({
      active: true
    }, function(err, found) {
      ...
      if (found < 1) {
        getFirstTheme(function(found) {
          compileIndex(found, GLOBAL.topshelfGlobals.extensions);
        });
      } else {
        compileIndex(found[0], GLOBAL.topshelfGlobals.extensions);
      }
    });
  }
}
...
function getFirstTheme(callback) {
  Themes.find({}, function(err, found) {
    if (err) {
      return console.log('error finding themes', err);
    }
    if (found.length < 1) {
      return console.log('could not find any themes');
    }
    if (callback) {
      callback(found[0]);
    }
...
function compileIndex(theme, extensionJSONS) {
  ...
  GLOBAL.topshelfGlobals.extensions = null;
