...
function populateGlobalRoles(roles) {
    ...
    GLOBAL.topshelfGlobals.roles = {}; //Clean up old values

    // Iterate through roles
    for (var i = 0; i < roles.length; i++) {
      GLOBAL.topshelfGlobals.roles[roles[i].role] = [];
      ...
      for (var permission in roles[i].permissions) {
        if (roles[i].permissions.hasOwnProperty(permission)) {
          ...
          if (roles[i].permissions[permission] == true) {
            GLOBAL.topshelfGlobals.roles[roles[i].role].push(permission);
          }
        }
      } //Iterate through permissions in each role
    } //Iterate through roles
    // console.log('GLOBAL.topshelfGlobals.roles', GLOBAL.topshelfGlobals.roles);
  } //populateGlobalRoles()
